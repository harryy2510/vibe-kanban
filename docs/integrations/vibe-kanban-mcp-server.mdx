...
| `update_issue` | Update an issue's title, description, status, or priority. Use `list_project_statuses` to find valid status names | `issue_id` | `title`<br/>`description`<br/>`status`<br/>`priority`<br/>`parent_issue_id` | Updated issue details |
